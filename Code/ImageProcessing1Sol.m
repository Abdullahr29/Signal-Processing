...
% the largest onto element 256. To achieve this, use the following:
...
image(X(:,:,:,1)); colormap(map); axis equal

% First, let's crop the image to exclude objects outside the field of view of the scan
X2=X(1:355,75:530,:,:);
X2=squeeze(X2); % this removes that extra matrix dimension.
% a better cropping can be done if you spend time in this
image(X2(:,:,1)); axis equal
colorbar;

% now if you look closely at the colour bar, you can see that the bottom
% values are reserved for grayscale image, while the top values are
% reserved for colour doppler. the middle range are likly used for the
% wording and labelling. So we can just find the number of pixels in the
% top range and bottom range.

mask_colour=find(X2(:,:,1)>=133);
mask_gray=find(X2(:,:,1)<=100);
mask_others=find(X2(:,:,1)>100 & X2(:,:,1)<133);

num_pix_colour=length(mask_colour)
num_pix_gray=length(mask_gray)

Xcolour=X2(:,:,1);
Xcolour(mask_gray)=0; Xcolour(mask_others)=0;

Xgray=X2(:,:,1);
Xgray(mask_colour)=0; Xgray(mask_others)=0;

figure();
image(Xcolour);colormap(map)

figure();
image(Xgray);colormap(map)
